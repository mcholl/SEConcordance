...
#se_body = "&gt; Genesis 2:2 וַיְכַל אֱלֹהִים בַּיֹּום הַשְּׁבִיעִי מְלַאכְתֹּו אֲשֶׁר עָשָׂה וַיִּשְׁבֹּת בַּיֹּום הַשְּׁבִיעִי מִכָּל־מְלַאכְתֹּו אֲשֶׁר עָשָֽׂה׃ The word =KJV"
...
def chunker(se_body, nchunk_size):
	nchunk_start=0
	...
	print len(refparser_params['text'])
	print sys.getsizeof(refparser_params['text'])
	print len(refparse.url)
	#So, the url is 2333 characters long, even though the length of the text is only 1500 characters.
	#The problem is clearly the encoding, and it may actually be more about what requests is doing then the actual python strings...
	print refparse.request.headers

	#Maybe I could add in a header?
